...
def propagate_correction(sim_data: pd.DataFrame, obs_data: pd.DataFrame, sim_data_to_correct,
                         drop_outliers: bool = False, outlier_threshold: int or float = 2.5) -> pd.DataFrame:
...
downstream_prop_correct = propagate_correction(start_flow, start_ideam_flow, downstream_flow, drop_outliers=True, outlier_threshold=.5)

sim_dates = downstream_flow.index.tolist()
scatters = [
    go.Scatter(
        name='Propagated Corrected (Experimental)',
        x=downstream_prop_correct.index.tolist(),
        y=downstream_prop_correct['Propagated Corrected Streamflow'].values.flatten(),
    ),
    go.Scatter(
    ...
    go.Scatter(
        name='Percentile',
        x=sim_dates,
        y=downstream_prop_correct['MonthlyPercentile'].values.flatten(),
    ),
    go.Scatter(
        name='Scalar',
        x=sim_dates,
        y=downstream_prop_correct['Scalars'].values.flatten(),
    ),
]
