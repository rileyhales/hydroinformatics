...
class CreateWeightTableFromECMWFRunoff(object):
    ...
    def getParameterInfo(self):
        ...
        param1 = arcpy.Parameter(name="in_rapid_connect_file",
    ...
    def execute(self, parameters, messages):
        ...
        # Calculate the geodesic area in square meters for each intersected polygon (no need to project if not already)
        ...
        # streamID_unique_arr = [int(row[0]) for row in list(connectivity_table)]
        streamID_unique_arr = []
        for row in list(connectivity_table):
            if row != []:
                streamID_unique_arr.append(row[0])


        weight_table_arr = [[streamID, 'area_sqm', 'lon_index', 'lat_index', 'npoints', 'weight', 'Lon', 'Lat']]
        for streamID_unique in streamID_unique_arr:
            ind_points = np.where(area_arr[streamID] == streamID_unique)[0]
            num_ind_points = len(ind_points)
            # get the total area
            area_geo_total = 0
            for ind_point in ind_points:
                area_geo_total += area_arr['AREA_GEO'][ind_point]

            arcpy.AddMessage("finished with the first section of the for loop")
            arcpy.AddMessage('the streamID being used is')
            arcpy.AddMessage(streamID_unique)
            arcpy.AddMessage("the length of ind_points is")
            arcpy.AddMessage(num_ind_points)
            arcpy.AddMessage('the ind point is')
            arcpy.AddMessage(ind_points)

            if num_ind_points <= 0:
                # if point not in array append dummy data for one point of data
                lon_each = area_arr['POINT_X'][0]
                lat_each = area_arr['POINT_Y'][0]
                ...
                row = [streamID_unique, 0, index_lon_each, index_lat_each, 1, 1.0, lon_each, lat_each]
                weight_table_arr.append(row)
                ...
                for ind_point in ind_points:
                    ...
                    weight_table_arr.append(row)
            arcpy.AddMessage('appended a row to weight table. the row was')
            arcpy.AddMessage(row)

        # Output the weight table
        with open(out_WeightTable, 'w') as csvfile:
            connectwriter = csv.writer(csvfile, dialect='excel')
            connectwriter.writerows(weight_table_arr)
